...
def preprocessing_tabular(path):
    ...
    # Repair table lines, sort contours, and extract ROI (Region of interest)
...
# Sheet1
# A2:B7
# Download the *.db file and open it
...
# planning to integrate that range function from xlsx module into the image module so multiple table can be extracted from the image at the same time
# improving accuracy for the image text detection, probably using self made model
